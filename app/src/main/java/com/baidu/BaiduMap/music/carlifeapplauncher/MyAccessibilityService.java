package com.baidu.BaiduMap.music.carlifeapplauncher;
...
public class MyAccessibilityService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        if (Common.getFlavor(this) == FlavorMode.zs) {
            String pkgName = event.getPackageName().toString();
            if (pkgName.equals("com.baidu.carlife")) {
                AccessibilityNodeInfo rootNodeInfo  = getRootInActiveWindow();
                if(rootNodeInfo!=null)
                {
                    List<AccessibilityNodeInfo> targetNodeInfos = rootNodeInfo.findAccessibilityNodeInfosByText("一律允许使用这台计算机进行调试");
                    if (!targetNodeInfos.isEmpty()) {
                        AccessibilityNodeInfo targetNodeInfo = targetNodeInfos.get(0);
    ...
    private void lifeCycle() {
        if (OpenProvider.isConnected(this)) {
            Log.i(TAG, "LifeCycle Check: Carlife Service On");
            if (!connection) {
                onConnected();
            }
        } else {
            Log.i(TAG, "LifeCycle Check: Carlife Service Off");
            if (connection) {
                onDisconnected();
            }
        }
...
//        }
    ...
    public void dispatchClick(int displayId, float x, float y) {
        Path dot = new Path();
        dot.moveTo(x, y);
        GestureDescription.Builder b = new GestureDescription.Builder().setDisplayId(displayId).addStroke(new GestureDescription.StrokeDescription(dot, 0, 500L));
